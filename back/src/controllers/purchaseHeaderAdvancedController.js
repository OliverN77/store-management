...
exports.getHeaderWithLines = async (req, res) => {
  ...
  try {
    ...
    const headerResult = await pool.request()
      .input('No_', no)
      .query('SELECT * FROM [OLIVER SOLUTIONS S.A.S$Purchase Header] WHERE [No_]=@No_');
    const linesResult = await pool.request()
      .input('DocumentNo', no)
      .query('SELECT * FROM [OLIVER SOLUTIONS S.A.S$Purchase Line] WHERE [Document No_]=@DocumentNo');
    res.json({
