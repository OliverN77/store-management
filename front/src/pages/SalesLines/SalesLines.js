...
import { salesLineService, salesHeaderService, itemService } from '../../services/dataService';
...
const SalesLines = () => {
  ...
  useEffect(() => {
    console.log('SalesLines formData:', formData);
  }, [formData]);
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      const [linesResponse, headersResponse, itemsResponse] = await Promise.all([
        salesLineService.getAll(),
        salesHeaderService.getAll(),
        itemService.getAll()
      ]);
      
      console.log(`Sales Lines para usuario ${user.id}:`, linesResponse.data.length);
      setSalesLines(linesResponse.data);
  ...
  const handleInputChange = (e) => {
    ...
    console.log('SalesLines input change:', { name, value });
    
    setFormData(prev => {
      const updated = { ...prev, [name]: value };
      
      // Calcular automáticamente el importe de línea cuando cambian cantidad o precio
      if (name === 'Quantity' || name === 'UnitPrice' || name === 'LineDiscountPct') {
      ...
      if (name === 'No_' && value) {
        ...
        if (selectedItem) {
          ...
          updated.UnitPrice = selectedItem.UnitPrice || selectedItem['Unit Price'] || '';
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validación básica
    if (!formData.DocumentNo || !formData.LineNo || !formData.No_ || !formData.Quantity || !formData.UnitPrice) {
      setError('Por favor complete todos los campos requeridos (Documento, Línea, Producto, Cantidad, Precio)');
      return;
    }
    ...
    try {
      ...
      console.log(`Enviando datos de línea de venta para usuario ${user.id}:`, dataToSend);
      ...
      if (editingLine) {
        ...
        await salesLineService.update(lineId, dataToSend);
        console.log(`Sales Line ${lineId} actualizada para usuario ${user.id}`);
      } else {
        await salesLineService.create(dataToSend);
        console.log(`Sales Line creada para usuario ${user.id}`);
      }
      ...
      setError(`Error al guardar línea de venta: ${errorMessage}`);
      console.error('Error saving sales line:', err);
  ...
  const handleEdit = (line) => {
    ...
    setFormData({
      ...
      UnitPrice: (line['Unit Price'] || line.UnitPrice || '').toString(),
  ...
  const handleDelete = async (line) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar esta línea de venta?')) {
      try {
        const documentNo = line['Document No_'] || line.DocumentNo;
        const lineNo = line['Line No_'] || line.LineNo;
        const lineId = `${documentNo}-${lineNo}`;
        await salesLineService.delete(lineId);
        setSalesLines(salesLines.filter(l => 
          !((l['Document No_'] || l.DocumentNo) === documentNo && 
            (l['Line No_'] || l.LineNo) === lineNo)
        ));
        console.log(`Sales Line ${lineId} eliminada para usuario ${user.id}`);
        setError(null);
      } catch (err) {
        setError('Error al eliminar línea de venta');
        console.error('Error deleting sales line:', err);
      }
    }
  };
  ...
  // CORREGIDO: Función para obtener el siguiente número de línea
  const getNextLineNo = (documentNo) => {
    const linesForDoc = salesLines.filter(line => 
  ...
  // Actualizar el número de línea cuando cambia el documento
  ...
  if (loading) return <div className="loading">Cargando líneas de venta...</div>;

  return (
    <div className="sales-lines">
      <Header title={`Líneas de Venta - ${user?.firstName || 'Usuario'}`} />
      
      <div className="sales-lines-content">
          ...
          <div className="modal-overlay">
            <div className="modal">
              <div className="modal-header">
                <h3>{editingLine ? 'Editar Línea de Venta' : 'Nueva Línea de Venta'}</h3>
                  ...
                  <div className="input-group">
                    ...
                    >
                      ...
                      {salesHeaders.map(header => (
                        <option key={header['No_'] || header.No_} value={header['No_'] || header.No_}>
                          {header['No_'] || header.No_} - Cliente: {header['Sell-to Customer No_'] || header.SellToCustomerNo}
                        </option>
                      ))}
                    </select>
                  </div>
        ...
        <div className="sales-lines-header">
          <div className="sales-lines-actions">
        ...
        <div className="sales-lines-table">
            ...
            <tbody>
              {salesLines.map((line, index) => (
                <tr key={`${line['Document No_'] || line.DocumentNo}-${line['Line No_'] || line.LineNo}-${index}`}>
                  ...
                  <td>${Number(line['Unit Price'] || line.UnitPrice || 0).toLocaleString()}</td>
          ...
          {salesLines.length === 0 && (
            <div className="empty-state">
              <p>No tienes líneas de venta registradas</p>
              <Button onClick={() => setShowForm(true)}>+ Agregar primera línea de venta</Button>
            </div>
          )}
        </div>
      </div>
